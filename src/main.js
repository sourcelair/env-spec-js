...
const checkValidationOfValues = envSpecString => {
  ...
  envValArray = envValArray.filter( element =>{ //check for valid variables AND types
    if(element[0].match(alphanumericThatDoesNotStartWithDigit &&(validTypes.includes(element[1]) || checkRestrValid(element[1]))){
      return element;
    }
    else{checkValidation = false;}
...
const checkRestrValid = restrField => {
  if(restrField[0]==='[' && restrField[restrField.length-1]===']'){
    restrChoices = restrField.split(",");
    return [true,restrChoices];
  }
  return false;
}
