...
test("Valid input with type", () => {
  ...
  return expect(envSpec.parse(testEnv)).resolves.toEqual(
...
test("Invalid environmental variable : starts with number", () => {
  ...
  return expect(envSpec.parse(testEnv)).rejects.toEqual("Error:Wrong Syntax");
});
...
test("Multiple invalid variables and types", () => {
  ...
  return expect(envSpec.parse(testEnv3)).rejects.toEqual("Error:Wrong Syntax");
});
