...
test("Valid input with type", () => {
  ...
  expect(envSpec.parse(testEnv)).resolves.toEqual(
...
test("Invalid environmental variable : starts with number", () => {
  ...
  expect(envSpec.parse(testEnv)).rejects.toEqual("Error:Wrong Syntax");
});
...
test("Multiple invalid variables and types", () => {
  ...
  expect(envSpec.parse(testEnv3)).rejects.toEqual("Error:Wrong Syntax");
});
