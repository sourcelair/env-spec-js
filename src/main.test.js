...
test("Valid input with type", () => {
  ...
  expect(envSpecToHTML(testEnv)).resolves.toEqual(
...
test("Invalid environmental variable : starts with number", () => {
  ...
  expect(envSpecToHTML(testEnv)).resolves.toEqual("Error:Wrong Syntax");
});
...
test("Multiple invalid variables and types", () => {
  ...
  expect(envSpecToHTML(testEnv3)).resolves.toEqual("Error:Wrong Syntax");
});
